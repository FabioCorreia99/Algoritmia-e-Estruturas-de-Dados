...
windows.title("Exercicio 3")
windows.geometry("800x600")

frame_txt = LabelFrame(windows, width=300, height=350, relief="sunken")
frame_txt.place(x=10,y=20)

todolist= Listbox(frame_txt, width=33,height=19)
todolist.place(x=10,y=20)

frame_input = LabelFrame(windows, width=300, height=100, relief="sunken")
...
btn_add = Button(windows,width=10, text="Adicionar", command=Adicionar)
btn_add.place(x=360,y=250)

btn_remove = Button(windows,width=10, text="remove", command=remover)
btn_remove.place(x=450,y=250)

btn_clear = Button(windows,width=10, text="clear", command=removertudo)
btn_clear.place(x=540,y=250)

btn_upload = Button(windows,width=10, text="Upload", command=upload)
btn_upload.place(x=360,y=300)

btn_download = Button(windows,width=10, text="Download", command=download)
btn_download.place(x=450,y=300)

btn_download = Button(windows,width=10, text="Ordenar", command=ordenar)
btn_download.place(x=540,y=300)

Tpen_label = Label(windows,text="Nº de Tarefas Pendentes:",fg="blue")
Tpen_label.place(x=350,y=350)

Tpen_txt = Text(windows,width=3,height=1)
